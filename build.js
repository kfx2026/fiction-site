...
<!-- Reader Toolbar -->
<div id="reader-toolbar" style="position:sticky;top:48px;z-index:50;background:rgba(10,11,16,0.95);border-bottom:1px solid rgba(255,255,255,0.06);backdrop-filter:blur(10px);padding:8px 0">
  <div style="max-width:720px;margin:0 auto;padding:0 16px;display:flex;align-items:center;justify-content:space-between;gap:4px">
    <div style="display:flex;align-items:center;gap:2px">
      <button onclick="readerFontDown()" title="Smaller text" style="padding:6px 10px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--dim);cursor:pointer;font-size:0.75rem;font-family:inherit">A−</button>
      <span id="reader-fs-val" style="font-size:0.65rem;color:var(--dim);min-width:36px;text-align:center">18px</span>
      <button onclick="readerFontUp()" title="Larger text" style="padding:6px 10px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--dim);cursor:pointer;font-size:0.75rem;font-family:inherit">A+</button>
    </div>
    <div style="display:flex;align-items:center;gap:2px">
      <button id="reader-theme-btn" onclick="readerToggleTheme()" title="Toggle theme" style="padding:6px 10px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--dim);cursor:pointer;font-size:0.9rem;font-family:inherit">🌙</button>
      <button id="reader-mode-btn" onclick="readerToggleMode()" title="Page / Scroll mode" style="padding:6px 10px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--dim);cursor:pointer;font-size:0.9rem;font-family:inherit">📜</button>
      <button id="reader-bm-btn" onclick="toggleBookmark()" title="Bookmark" style="padding:6px 10px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--dim);cursor:pointer;font-size:0.9rem;font-family:inherit">🔖</button>
      <button onclick="readerToggleChapter()" title="Chapter list" style="padding:6px 10px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--dim);cursor:pointer;font-size:0.9rem;font-family:inherit">☰</button>
    </div>
  </div>
</div>

<!-- Page Navigation (visible only in page mode) -->
<div id="reader-page-nav" style="display:none;max-width:720px;margin:0 auto;padding:4px 16px;text-align:center">
  <span style="display:inline-flex;align-items:center;gap:12px;font-family:-apple-system,BlinkMacSystemFont,sans-serif">
    <button onclick="prevPage()" id="reader-page-prev" style="padding:4px 12px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--dim);cursor:pointer;font-size:0.72rem;font-family:inherit">◀</button>
    <span id="reader-page-ofs" style="font-size:0.7rem;color:var(--dim)">1/1</span>
    <button onclick="nextPage()" id="reader-page-next" style="padding:4px 12px;border-radius:6px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:var(--dim);cursor:pointer;font-size:0.72rem;font-family:inherit">▶</button>
  </span>
</div>

<!-- Chapter Drawer -->
<div id="reader-drawer" style="position:fixed;top:0;right:0;bottom:0;width:300px;max-width:80vw;background:var(--card);z-index:200;transform:translateX(100%);transition:transform 0.25s cubic-bezier(0.4,0,0.2,1);box-shadow:-4px 0 24px rgba(0,0,0,0.3);overflow-y:auto">
  <div id="reader-ch-list"></div>
</div>
<div id="reader-drawer-overlay" onclick="toggleChapterDrawer()" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.4);z-index:199"></div>
<script>
(function(){
  var d=document.getElementById('reader-drawer');
  var ov=document.getElementById('reader-drawer-overlay');
  document.addEventListener('click',function(e){
    if(e.target.closest('#reader-drawer')||e.target.closest('[onclick*=\"readerToggleChapter\"]'))return;
    if(d.classList.contains('open')){d.classList.remove('open');ov.style.display='none';}
  });
  var obs=new MutationObserver(function(){
    var isOpen=d.classList.contains('open');
    d.style.transform=isOpen?'translateX(0)':'translateX(100%)';
...
<main class="cp">
  ...
  <div id="reader-content" class="ch-content">${text}</div>
  <nav class="cp-nav">
